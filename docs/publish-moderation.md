...
`updateName`/`resizeNonogram`, so cancelling leaves the in-memory board untouched. The predicate is
`GenViewModel.needsPublicEditConfirmation(changesContent)`, which reads the current puzzle's
`isPublic` itself.

`GenViewModel.setPublic` applies the author's switch — `APPROVED` ↔ `UNLISTED`, ignored in every
other state, because publication is the admin's call. It only stages the new status on the
in-memory puzzle (it is not an edit, so it never sets `isDirty`); the config screen's Save persists
it like any other change.

The publish control itself has no separate model: `GenConfScreen` branches straight on
`PublishStatus` — the switch for `APPROVED`/`UNLISTED`, "Denied" for `DENIED`, and otherwise the
request button, disabled and labelled "Sent" while `PENDING`. For `NONE` the button is enabled only
when the author is not banned, is signed in, and the puzzle validated; the same chain picks the hint
under it.
...
The review queue is a plain `List<Nonogram>`. `Nonogram.authorUid` holds the same string Firestore's
`authorUid` field does, so a pending request needs no wrapper type to carry its author, and both
`parseNonograms` implementations are the single parse path for public pulls, owned pulls, and the
queue alike.
