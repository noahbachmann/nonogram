...
actual fun GoogleSignInSection(
    ...
    val googleSignIn = rememberGoogleSignInState { result ->
        result
            .onSuccess { googleUser ->
                val idToken = googleUser.idToken.takeIf { it.isNotBlank() }
                val accessToken = googleUser.accessToken?.takeIf { it.isNotBlank() }
            ...
            .onFailure { error ->
                if (error !is KMPAuthUserCancelledException) {
                    println("GoogleSignIn(web): sign-in failed: ${error.message}")
                }
            }
    }
